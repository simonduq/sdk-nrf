...
This sample is a minimal, error tolerant, integrated demonstration of the :ref:`lib_nrf_cloud`, and :ref:`lib_location` libraries.
...
* Periodic Wi-Fi® and location tracking using the :ref:`lib_location` library.
...
This includes generation of false temperature readings.
...
When enabled, the device scans the MAC addresses of nearby access points and submits them to nRF Cloud to obtain a location estimate.
...
+------------------------------------------------------+--------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------+
| Connection to nRF Cloud lost, reconnecting           | Single LED lit, spinning around the square of LEDs                                                     | The :ref:`CONFIG_LED_VERBOSE_INDICATION <CONFIG_LED_VERBOSE_INDICATION>` option is enabled                |
+------------------------------------------------------+--------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------+
| Fatal error                                          | All four LEDs blinking, 75% duty cycle                                                                 | LED status indication is enabled                                                                          |
+------------------------------------------------------+--------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------+
| Device message sent successfully                     | Alternating checkerboard pattern (two LEDs are lit at a time, either LED1 and LED4, or LED2 and LED3)  | The :ref:`CONFIG_LED_VERBOSE_INDICATION <CONFIG_LED_VERBOSE_INDICATION>` option is enabled                |
+------------------------------------------------------+--------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------+
| Idle                                                 | Single LED lit, bouncing between opposite corners (LED1 and LED4)                                      | The :ref:`CONFIG_LED_CONTINUOUS_INDICATION <CONFIG_LED_CONTINUOUS_INDICATION>` option is enabled          |
+------------------------------------------------------+--------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------+
...
.. list-table::
   ...
   * - Feature
     - Kconfig option
   * - Location tracking
     - :ref:`CONFIG_LOCATION_TRACKING <CONFIG_LOCATION_TRACKING>`
...
For examples, see the related minimal overlays in the :ref:`wifi_nrf_cloud_minimal` section.
...
To see all debug output for this sample, enable the :ref:`CONFIG_WIFI_NRF_CLOUD_LOG_LEVEL_DBG <CONFIG_WIFI_NRF_CLOUD_LOG_LEVEL_DBG>` option.
...
.. _CONFIG_WIFI_NRF_CLOUD_LOG_LEVEL_DBG:

CONFIG_WIFI_NRF_CLOUD_LOG_LEVEL_DBG - Sample debug logging
...
CONFIG_DATE_TIME_ESTABLISHMENT_TIMEOUT_SECONDS - Date and time establishment timeout (seconds)
   Sets the timeout for date and time establishment (in seconds).
   The sample waits for this number of seconds for the device to determine the current date and time before giving up and moving on.
...
CONFIG_LED_INDICATION_GPIO - GPIO LED indication
   ...
   Defaults to enabled if there is a compatible devicetree entry.
...
.. _wifi_nrf_cloud_building_provisioning_service:
...
.. note::
   ...
   This service is not compatible yet with nRF7x devices.
...
Once the sample is built and flashed, proceed to :ref:`wifi_nrf_cloud_provisioning_service` for instructions on how to provision your device with the Provisioning Service.
...
.. _wifi_nrf_cloud_building_wifi_conn:

Building with experimental support for Wi-Fi connectivity
=========================================================

This sample :ref:`experimentally <software_maturity>` supports connecting to nRF Cloud using Wi-Fi.
...
See the :file:`src/prj.conf` configuration file and the :file:`overlay_nrf700x_wifi_mqtt.conf` overlay for additional details.
...
.. important::
   ...
   Even though this is more secure than :ref:`hard-coded credentials <wifi_nrf_cloud_build_hardcoded>`, the device private key still has to be loaded into unprotected memory during TLS connections.
...
If you are certain you understand the risks, you can configure your build to use Wi-Fi connectivity using the ``-DSB_CONF_FILE=sysbuild_nrf700x-wifi-conn.conf`` and ``-DEXTRA_CONF_FILE=overlay_nrf700x_wifi_mqtt.conf`` options.
            ...
            .. tabs::
               ...
               .. group-tab:: Bash
                  ...
                  .. code-block:: bash
                     ...
                     west build --board nrf5340dk/nrf5340/cpuapp/ns -p always -- -DSHIELD=nrf7002ek -DEXTRA_CONF_FILE=overlay_nrf700x_wifi_mqtt.conf -DSB_CONF_FILE=sysbuild_nrf700x-wifi-conn.conf -DCONFIG_NRF_CLOUD_CLIENT_ID_SRC_COMPILE_TIME=y -DCONFIG_NRF_CLOUD_CLIENT_ID=\"698d4c11-0ccc-4f04-89cd-6882724e3f6f\"
            ...
            .. tabs::
               ...
               .. group-tab:: Bash
                  ...
                  .. code-block:: bash
                     ...
                     west build --board nrf5340dk/nrf5340/cpuapp/ns -p always -- -DSHIELD=nrf7002ek -DEXTRA_CONF_FILE=overlay_nrf700x_wifi_coap.conf -DSB_CONF_FILE=sysbuild_nrf700x-wifi-conn.conf -DCONFIG_NRF_CLOUD_CLIENT_ID_SRC_COMPILE_TIME=y -DCONFIG_NRF_CLOUD_CLIENT_ID=\"698d4c11-0ccc-4f04-89cd-6882724e3f6f\"
            ...
            .. tabs::
               ...
               .. group-tab:: Bash
                  ...
                  .. code-block:: bash
                     ...
                     west build --board nrf7002dk/nrf5340/cpuapp/ns -p always -- -DEXTRA_CONF_FILE=overlay_nrf700x_wifi_mqtt.conf -DSB_CONF_FILE=sysbuild_nrf700x-wifi-conn.conf -DCONFIG_NRF_CLOUD_CLIENT_ID_SRC_COMPILE_TIME=y -DCONFIG_NRF_CLOUD_CLIENT_ID=\"698d4c11-0ccc-4f04-89cd-6882724e3f6f\"
            ...
            .. tabs::
               ...
               .. group-tab:: Bash
                  ...
                  .. code-block:: bash
                     ...
                     west build --board nrf7002dk/nrf5340/cpuapp/ns -p always -- -DEXTRA_CONF_FILE=overlay_nrf700x_wifi_coap.conf -DSB_CONF_FILE=sysbuild_nrf700x-wifi-conn.conf -DCONFIG_NRF_CLOUD_CLIENT_ID_SRC_COMPILE_TIME=y -DCONFIG_NRF_CLOUD_CLIENT_ID=\"698d4c11-0ccc-4f04-89cd-6882724e3f6f\"
...
Once the sample is built and flashed, proceed to :ref:`wifi_nrf_cloud_standard_onboarding` for instructions on how to onboard your device.

Once your device is onboarded, proceed to :ref:`wifi_nrf_cloud_setup_wifi_cred` for instructions on configuring your device to connect to a specific access point.
...
.. note::
   ...
   See :ref:`wifi_nrf_cloud_provisioning_onboarding` for a list of other methods.
...
.. _wifi_nrf_cloud_provisioning_onboarding:
...
The following sections outline the various onboarding methods that this sample supports.
  ...
  * If you are building with :ref:`provisioning service support <wifi_nrf_cloud_building_provisioning_service>`, proceed to :ref:`wifi_nrf_cloud_provisioning_service`.
    This method is not compatible yet with nRF7x devices.
  * If you are not building with Provisioning Service support, proceed to :ref:`wifi_nrf_cloud_standard_onboarding`.
    Use this method preferably.
...
.. _wifi_nrf_cloud_provisioning_service:

Provisioning with the nRF Cloud Provisioning Service
====================================================

The nRF Cloud Provisioning Service is not compatible yet with nRF7x devices.
For now, proceed to :ref:`wifi_nrf_cloud_standard_onboarding`.

.. important::
   Your project needs to be built with :ref:`Provisioning service support <wifi_nrf_cloud_building_provisioning_service>` otherwise the next steps will not work. See `Provisioning Service <nRF Cloud Provisioning Service_>`_ for more detail information.
...
* Using scripted onboarding, as follows:
   ...
   #. Use the :file:`claim_and_provision_device` script from :ref:`nRF Cloud Utils <wifi_nrf_cloud_install_nrf_utils>` to provision and onboard your device
      ...
      When the script has successfully completed, the device appears in the `Devices <nRF Cloud Portal Devices_>`_ list in the nRF Cloud portal and demonstrates the :ref:`supported nRF Cloud features <wifi_nrf_cloud_features>`.
      ...
      .. group-tab:: MQTT
         ...
         Where:
           ...
           * *device_id* is the (globally unique) ID for your device.
             ...
             See :ref:`wifi_nrf_cloud_building_wifi_conn` for details.
