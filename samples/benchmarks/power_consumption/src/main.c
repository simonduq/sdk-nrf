...
#define IDLE_TIME K_SECONDS(CONFIG_POWER_CONSUMPTION_IDLE_SECONDS)

static void configure_ram_retention(void)
{
#if defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_UNUSED_ONLY)
	power_down_unused_ram();
#elif defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_64K) || \
	defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_128K) || \
	defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_256K) || \
	defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_512K)
	uintptr_t ram_start = DT_REG_ADDR(DT_CHOSEN(zephyr_sram));
	uintptr_t ram_end = ram_start + DT_REG_SIZE(DT_CHOSEN(zephyr_sram));

#if defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_64K)
	uintptr_t retained_size = 64U * 1024U;
#elif defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_128K)
	uintptr_t retained_size = 128U * 1024U;
#elif defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_256K)
	uintptr_t retained_size = 256U * 1024U;
#elif defined(CONFIG_POWER_CONSUMPTION_RAM_RETAIN_512K)
	uintptr_t retained_size = 512U * 1024U;
#endif

	power_down_ram(ram_start + retained_size, ram_end);
#endif
	/* CONFIG_POWER_CONSUMPTION_RAM_RETAIN_FULL (default): no power-down
	 * call at all -- every byte of application RAM stays retained.
	 */
}
