...
int coap_codec_sensor_encode(const char *app_id, double float_val, int64_t ts, uint8_t *buf,
			     size_t *len, enum coap_content_format fmt)
{
	...
	struct nrf_cloud_obj_coap_cbor msg = {.app_id = (char *)app_id,
...
int coap_codec_pvt_encode(const char *app_id, const struct nrf_cloud_gnss_pvt *pvt, int64_t ts,
			   ...
			   struct lte_lc_cells_info const *const cell_info)
{
	...
	if (cell_info->current_cell.id != LTE_LC_CELL_EUTRAN_ID_INVALID) {
		...
		nmrs = MIN(ARRAY_SIZE(enc_cell->cell_nmr.cell_nmr_ncells), cell_info->ncells_count);
		if (nmrs) {
			copy_ncells(enc_cell->cell_nmr.cell_nmr_ncells, nmrs,
...
int coap_codec_ground_fix_req_encode(struct lte_lc_cells_info const *const cell_info,
				     struct wifi_scan_info const *const wifi_info, uint8_t *buf,
				     size_t *len, enum coap_content_format fmt)
{
...
int coap_codec_ground_fix_resp_decode(struct nrf_cloud_location_result *result, const uint8_t *buf,
				      size_t len, enum coap_content_format fmt)
{
				 ...
				 size_t len, enum coap_content_format fmt)
{
	...
	if (result->buf_sz < len) {
		LOG_WRN("A-GNSS buffer is too small; expected: %zd, got:%zd; truncated", len,
			result->buf_sz);
	}
